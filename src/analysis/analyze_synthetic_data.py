...
if __name__ == "__main__":
    # --- Load processed synthetic data from database ---
    print("📥 Loading synthetic data from database...")
    loader = SyntheticDataLoader()
    df = loader.load_data_from_db()

    if df.empty:
        print("⚠️ No data found in database. Please run synthetic_pipeline.py first.")
        sys.exit(1)
    ...
    output_path = PROJECT_ROOT / "data" / "processed" / "analyzed_synthetic_data.csv"
    ...
    for factor, data in risk_stats.items():
        if isinstance(data, dict) and 'percentage' in data:
            print(f"{data['description']}: {data['measures_of_central_tendency']} cases ({data['percentage']:.1f}%)")
    ...
    print(catboost_classifier.feature_importance().head())

    print("XGBoost Feature Importance:")
    print(xgboost_classifier.feature_importance().head())
